...
sudo docker run --name ${DOCKER_CONTAINER_NAME} -d -e "PYTHONPATH=$DOCKER_PROJECT_PATH"  --restart=always --log-opt max-size=50m --log-opt max-file=5 ${DOCKER_IMAGE_NAME}
